...
$action = $isEdit
    ...
    : ($bulkMode ? site_url('Content/Personal/Create') : site_url('Content/Personal/Create') . '?recipient_id=' . (int) ($message['recipient_id'] ?? 0));
...
<h1><?= $isEdit ? 'Edit Personal Message' : ($bulkMode ? 'Create Bulk Personal Message' : 'Create Personal Message') ?></h1>
<p class="lead"><?= $isEdit ? 'Update a message you sent.' : ($bulkMode ? 'Send one personal message to every active user.' : 'Send a private one-to-one message to the selected member.') ?></p>
...
<form method="post" action="<?= esc($action) ?>" class="card">
    ...
    <div class="wysiwyg" data-wysiwyg>
        <div class="wysiwyg-toolbar" aria-label="Personal message editor toolbar">
    ...
    <p class="hint">Use the toolbar for rich messages. HTML source mode is available for fine adjustments.</p>
...
(function () {
    ...
    const recipient = document.querySelector('.readonly-field');
    ...
    function showPreview() {
        if (! preview || ! previewTitle || ! previewRecipient || ! previewBody) {
            return;
        }

        const subject = (document.getElementById('subject')?.value || '').trim() || 'Untitled preview';
        const body = sourceMode ? source.value.trim() : editor.innerHTML.trim();

        previewTitle.textContent = subject;
        previewRecipient.textContent = 'Recipient: ' + (recipient ? recipient.textContent.trim() : 'Selected user');
        previewBody.innerHTML = body || '<p>No message content yet.</p>';
